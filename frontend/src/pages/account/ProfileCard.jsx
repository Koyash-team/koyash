...
const ROWS = [
  ...
  { y: 744, label: 'Аллергии', icon: icAllergy, size: 60 },
...
// Before the questionnaire the rows carry labels only, and an intro line sits
// above them (Figma «без подбора»), so they use a lower, tighter layout.
const NOPICK_Y = [620, 675, 730, 785, 840, 895, 950];
...
export default function ProfileCard({
  ...
  return (
    <>
      ...
      {/* Before the questionnaire: intro line + heart divider above the labels */}
      {!hasProfile && (
        <>
          <p
          ...
          <img
            ...
            alt=""
            aria-hidden="true"
            style={{ left: 59, top: 586, width: 384, height: 30, objectFit: 'contain' }}
          />
        </>
      )}

      {ROWS.map((row, i) => {
        const y = hasProfile ? row.y : NOPICK_Y[i];
        return (
          <div key={row.label}>
            {row.icon && (
              <span
